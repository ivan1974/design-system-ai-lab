...
The goal is to choose the simplest visual form that helps the user compare,
understand, verify or act.
...
If a visualization does not support one of these tasks, use text, rows or a
compact metric instead.
...
Use a table or dense list when the user needs to scan, filter or compare many
items.
...
Use compact metrics when several operational signals need to be visible without
...
Use topology or single-line diagram representations only when requested or when
asset relationships are the primary decision.

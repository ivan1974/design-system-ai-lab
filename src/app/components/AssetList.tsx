...
// §6 v1.1: column 6 is now "Status" (renamed from "Activity")
...
export function AssetList({ assets, selectedId, onSelect }: AssetListProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-neutral-50/40">
      ...
      <div
        className={`sticky top-0 z-20 bg-white border-b border-neutral-200 grid ${COL_WIDTHS} px-5 gap-3`}
        ...
        {groups.map(group => (
          <div key={`${group.building}||${group.area}||${group.room}`}>
            <div className="sticky top-[41px] z-10 px-5 py-2 flex items-center gap-1.5 bg-neutral-50/95 border-b border-neutral-100">
              ...
              <span className="ml-1.5 text-[10px] px-1.5 py-0.5 rounded-full bg-neutral-200 text-neutral-500" style={{ fontWeight: 600 }}>
                {group.assets.length}
              </span>
            </div>
...
function AssetRow({ asset, selected, onSelect }: { asset: Asset; selected: boolean; onSelect: () => void }) {
  return (
    <div
      onClick={onSelect}
      className={`grid ${COL_WIDTHS} px-5 gap-3 cursor-pointer transition-all duration-100 group relative ${
        selected ? 'bg-[#f0faf5]' : 'hover:bg-neutral-50/80'
      }`}
    >
      {selected && <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-[#00985F] rounded-r" />}

      {/* 1. Asset: Connectivity + Name + Reference + Description */}
      ...
      {/* 2. Type — §8 v1.1 Style: Tag */}
      <div className="py-3.5 flex items-center">
        <TypeTag type={asset.type} />
      </div>

      {/* 3. Location: Building + Electrical Area + Room */}
      ...
      {/* 4. Coverage: Service Coverage + DPP Status */}
      ...
      {/* 6. Status (renamed from Activity) — §12 v1.1 */}
      ...
      <div className="py-3.5 flex items-center justify-end">
        <div className={`w-6 h-6 rounded-md flex items-center justify-center transition-all duration-150 ${
          selected ? 'bg-[#00985F] text-white' : 'text-neutral-300 group-hover:text-neutral-500 group-hover:bg-neutral-100'
        }`}>
...
// §12 v1.1: renamed from ActivityLabel to StatusLabel
